...
#
#     return epoch_loss
def evaluate_epoch_with_images(model, param, data_loader):
    """评估并返回损失和平均SSIM"""
    ...
    with torch.no_grad():
        for iter, (images, _) in enumerate(data_loader):
            ...
            # 直接在当前batch计算SSIM
            # 确保图像在[0,1]范围内
            images_for_ssim = images.clone()
            outputs_for_ssim = outputs.clone()

            if images_for_ssim.max() > 1.0:
                images_for_ssim = images_for_ssim / 255.0
            if outputs_for_ssim.max() > 1.0:
                outputs_for_ssim = outputs_for_ssim / 255.0

                # 计算当前batch的SSIM
            batch_ssim = ssim(outputs_for_ssim, images_for_ssim, data_range=1.0, size_average=True)
            ssim_sum += batch_ssim.item()
            count += 1

            # 计算MSE损失
            outputs_denorm = image_normalization('denormalization')(outputs)
            images_denorm = image_normalization('denormalization')(images)
            loss = model.loss(images_denorm, outputs_denorm) if not param['parallel'] else model.module.loss(
                images_denorm, outputs_denorm)
            epoch_loss += loss.detach().item()

        epoch_loss /= (iter + 1)
        avg_ssim = ssim_sum / count

    return epoch_loss, avg_ssim
