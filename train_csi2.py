...
import matplotlib.pyplot as plt  # === 修改：新增绘图库 ===
...
def evaluate_epoch(model, param, data_loader):
    ...
    with torch.no_grad():
        for images, _ in data_loader:
            images = images.cuda() if param['parallel'] and torch.cuda.device_count() > 1 else images.to(
                param['device'])
...
def train_pipeline(params):
    ...
    # Early stopping (Keep initialization but will not use trigger)
    ...
    try:
        with tqdm(range(params['epochs']), disable=params['disable_tqdm']) as t:
            for epoch in t:
                ...
                # === 修改：记录数据到 history ===
                history['train_loss'].append(train_metrics['img_loss'])
                history['val_loss'].append(val_metrics['img_loss'])
    ...
    # === 修改：绘制并保存 Loss 和 PSNR 曲线 ===
    try:
        # 1. 绘制 Loss 曲线
        plt.figure(figsize=(10, 5))
        plt.plot(history['train_loss'], label='Train Loss')
        plt.plot(history['val_loss'], label='Val Loss')
        plt.title(f"Loss Curve (SNR={params['snr']}dB, Ratio={params['ratio']:.2f}, Ch={params['channel']})")
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
        plt.grid(True)
        loss_plot_path = os.path.join(root_log_dir, 'loss_curve.png')
        ...
        plt.title(f"PSNR Curve (SNR={params['snr']}dB, Ratio={params['ratio']:.2f}, Ch={params['channel']})")
        plt.xlabel('Epoch')
        plt.ylabel('PSNR (dB)')
        plt.legend()
        plt.grid(True)
        psnr_plot_path = os.path.join(root_log_dir, 'psnr_curve.png')
