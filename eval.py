...
def process_config(config_path, output_dir, dataset_name, times):
    ...
    if dataset_name == 'cifar10':
        ...
        test_loader = DataLoader(test_dataset, shuffle=True,
                                 batch_size=params['batch_size'], num_workers=params['num_workers'])
    elif dataset_name == 'kodak':  # 添加KODAK数据集支持
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Resize((128, 128))  # 与ImageNet保持一致尺寸
        ])
        # 假设KODAK数据集图片存放在 ../dataset/KODAK/ 目录下
        test_dataset = Vanilla(root='./dataset/kodak', transform=transform)
        test_loader = DataLoader(test_dataset, shuffle=False,  # KODAK通常不打乱以保持一致性
                                 batch_size=params['batch_size'], num_workers=params['num_workers'])
    else:
...
def main():
    ...
    config_files = [os.path.join(config_dir, name) for name in os.listdir(config_dir)
                    if
                    (dataset_name in name or dataset_name.upper() in name) and channel_type in name and name.endswith(
                        '.yaml')]

    with ProcessPoolExecutor() as executor:
        executor.map(process_config, config_files, [output_dir] * len(config_files), [dataset_name] * len(config_files),
