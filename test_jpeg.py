...
CONFIG = {
    ...
    "jpeg_performance": {
        "snr_10": 10,  # SNR=10时的点
        "psnr_10": 12.08,  # PSNR值
        "msssim_10": 0.4405,  # MS-SSIM值
        "color": "black",  # 曲线颜色
        "marker": "o",  # 标记形状
        "linestyle": ":",  # 线型（虚线）
        "label": "JPEG (Cliff Effect)",  # 标签
        "cliff_marker_size": 12,  # 悬崖点标记大小
    },
    ...
    "models": [
        ...
        # {
        #     "path": "./out/checkpoints/CIFAR10_8_7.0_0.17_AWGN_fb32_20h35m54s_on_Nov_27_2025/epoch_999.pkl",
        #     "type": "csi",
        #     "label": "JSCC+CSI SNR$_{train}$=7dB",
        #     "color": "green",
        #     "marker": "v",
        #     "linestyle": "-"
        # },
        # {
        #     "path": "./out/checkpoints/CIFAR10_8_7.0_0.17_AWGN_22h13m53s_on_Jun_07_2024/epoch_999.pkl",
        #     "type": "baseline",
        #     "label": "JSCC SNR$_{train}$=7dB",
        #     "color": "green",
        #     "marker": "s",
        #     "linestyle": "--"
        # },
...
def get_jpeg_results(snr_list, jpeg_cfg):
    """生成JPEG性能曲线数据"""
    jpeg_snr_10 = jpeg_cfg["snr_10"]
    jpeg_psnr_10 = jpeg_cfg["psnr_10"]
    jpeg_msssim_10 = jpeg_cfg["msssim_10"]
    ...
    for snr in snr_list:
        jpeg_results['snr'].append(snr)
        if snr < jpeg_snr_10:
            # SNR<10时，性能为0
            jpeg_results['psnr'].append(0)
            jpeg_results['msssim'].append(0)
        elif snr == jpeg_snr_10:
            # SNR=10时，有具体值
            jpeg_results['psnr'].append(jpeg_psnr_10)
            jpeg_results['msssim'].append(jpeg_msssim_10)
        else:
            # 对于大于10的SNR，保持相同的值（或可设为0）
            jpeg_results['psnr'].append(0)
            jpeg_results['msssim'].append(0)
...
def plot_curves(all_results, model_configs, out_dir, channel_type, ratio, jpeg_cfg):
    """绘制专业对比曲线图"""
    ...
    # 先绘制JPEG曲线
    snr_list = list(all_results.values())[0]['snr']
    jpeg_results = get_jpeg_results(snr_list, jpeg_cfg)

    # 绘制JPEG的悬崖效应：在SNR=10处画垂直线
    jpeg_snr_10 = jpeg_cfg["snr_10"]
    jpeg_psnr_10 = jpeg_cfg["psnr_10"]
    cliff_marker_size = jpeg_cfg.get("cliff_marker_size", 12)

    # 绘制垂直线表示悬崖效应
    ax.plot([jpeg_snr_10, jpeg_snr_10], [0, jpeg_psnr_10],
            color=jpeg_cfg["color"], linestyle='--', linewidth=2, alpha=0.7,
            label='_nolegend_')

    # 绘制JPEG曲线（大部分SNR为0，只有SNR=10有值）
    ax.plot(jpeg_results['snr'], jpeg_results['psnr'],
            marker=jpeg_cfg["marker"],
            linestyle=jpeg_cfg["linestyle"],
            linewidth=2,
            markersize=8,
            label=jpeg_cfg["label"],
            color=jpeg_cfg["color"])

    # 在悬崖点处标记特殊标记
    ax.plot(jpeg_snr_10, jpeg_psnr_10,
            marker='*', markersize=cliff_marker_size,
            color=jpeg_cfg["color"], markeredgecolor='red', markeredgewidth=2,
            label='_nolegend_')
    ...
    # 设置 Y 轴从 10 开始
    all_psnr_values = [jpeg_psnr_10] + [psnr for res in all_results.values() for psnr in res['psnr']]
    y_min = max(0, min(all_psnr_values) - 2)
    y_max = max(all_psnr_values) + 2
    ax.set_ylim(y_min, y_max)

    # 设置 X 轴范围
    x_values = list(all_results.values())[0]['snr']
    ...
    # 添加悬崖效应标注
    ax.annotate('Cliff Effect', xy=(jpeg_snr_10, jpeg_psnr_10 / 2),
                xytext=(jpeg_snr_10 - 2, jpeg_psnr_10 / 2 - 2),
    ...
    plt.savefig(os.path.join(out_dir, "kodak_psnr_curve.pdf"), bbox_inches='tight')
    plt.close()
    ...
    # 绘制垂直线表示悬崖效应
    ax.plot([jpeg_snr_10, jpeg_snr_10], [0, jpeg_msssim_10],
            color=jpeg_cfg["color"], linestyle='--', linewidth=2, alpha=0.7,
            label='_nolegend_')

    # 绘制JPEG曲线
    ax.plot(jpeg_results['snr'], jpeg_results['msssim'],
            marker=jpeg_cfg["marker"],
            linestyle=jpeg_cfg["linestyle"],
            linewidth=2,
            markersize=8,
    ...
    # 在悬崖点处标记特殊标记
    ax.plot(jpeg_snr_10, jpeg_msssim_10,
            marker='*', markersize=cliff_marker_size,
            color=jpeg_cfg["color"], markeredgecolor='red', markeredgewidth=2,
            label='_nolegend_')
    ...
    # 设置 Y 轴范围
    all_msssim_values = [jpeg_msssim_10] + [msssim for res in all_results.values() for msssim in res['msssim']]
    y_min_ssim = max(0, min(all_msssim_values) - 0.05)
    y_max_ssim = min(1.0, max(all_msssim_values) + 0.02)
    ax.set_ylim(y_min_ssim, y_max_ssim)
    ...
    # 添加悬崖效应标注
    ax.annotate('Cliff Effect', xy=(jpeg_snr_10, jpeg_msssim_10 / 2),
                xytext=(jpeg_snr_10 - 2, jpeg_msssim_10 / 2 - 0.05),
                arrowprops=dict(arrowstyle='->', color='red', lw=1.5),
                fontsize=11, color='red', fontweight='bold')

    plt.tight_layout()
    msssim_path = os.path.join(out_dir, "kodak_msssim_curve.png")
    plt.savefig(msssim_path, dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(out_dir, "kodak_msssim_curve.pdf"), bbox_inches='tight')
    plt.close()
    ...
    # 返回包含JPEG的结果
    all_results['JPEG (Cliff Effect)'] = jpeg_results
    return all_results
...
def main():
    ...
    if all_results:
        print("\n" + "=" * 70)
        print("所有模型评估完成，正在生成对比曲线图（包含JPEG悬崖效应）...")
        print("=" * 70)

        all_results = plot_curves(all_results, successful_models, cfg["out_dir"],
                                  cfg["channel"], cfg["ratio"], cfg["jpeg_performance"])
        ...
        print(f"数值结果已保存到: {results_path}")

        # 打印结果汇总表格
        print("\n" + "=" * 70)
        print("结果汇总（包含JPEG）")
        print("=" * 70)
        print(f"{'Model':<35} | {'SNR (dB)':<10} | {'PSNR (dB)':<12} | {'MS-SSIM':<10}")
        print("-" * 70)
        for name, res in all_results.items():
            for i, snr in enumerate(res['snr']):
                if i == 0:
                    print(f"{name:<35} | {snr:<10} | {res['psnr'][i]:<12.3f} | {res['msssim'][i]:<10.4f}")
                else:
                    print(f"{'':<35} | {snr:<10} | {res['psnr'][i]:<12.3f} | {res['msssim'][i]:<10.4f}")
            print("-" * 70)
    else:
        print("\n没有成功评估任何模型")
