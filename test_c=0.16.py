...
def plot_curves(all_results, model_configs, out_dir, channel_type, ratio):
    ...
    # 设置 Y 轴从 10 开始
    y_min = max(10, min([min(r['psnr']) for r in all_results.values()]) - 2)
    y_max = max([max(r['psnr']) for r in all_results.values()]) + 2
    ...
    # 设置 Y 轴范围
    y_min_ssim = max(0.3, min([min(r['msssim']) for r in all_results.values()]) - 0.05)
    y_max_ssim = min(1.0, max([max(r['msssim']) for r in all_results.values()]) + 0.02)
...
def main():
    ...
    if all_results:
        ...
        plot_curves(all_results, successful_models, cfg["out_dir"],
                    cfg["channel"], cfg["ratio"])
